...
namespace Yor
{
  ...
  Entity SceneRegistry::createEntity(const std::string& name)
  {
    if (!m_freeIndices.empty())
    {
      ...
      m_entities[oldIndex] = Entity(UUID(), this);
      m_entities[oldIndex].m_components.reset();
      m_entities[oldIndex].m_id = UUID();
      m_entities[oldIndex].m_registry = this;
      return m_entities[oldIndex];
    }
    else
    {
      m_entities[m_entityCounter].m_components.reset();
      m_entities[m_entityCounter].m_id = UUID();
      m_entities[m_entityCounter].m_registry = this;
